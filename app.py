...
with app.app_context():
        ...
        if 'inbox_imap_account' in inspector.get_table_names():
            ...
            with db.engine.connect() as conn:
                ...
                if 'is_enabled' not in columns:
                    conn.execute(text("UPDATE inbox_imap_account SET is_enabled = TRUE WHERE is_enabled IS NULL"))
                conn.commit()
    ...
    # Auto-migration: Add is_read / uid_validity columns to inbox_email_message
    try:
        from sqlalchemy import inspect as _ii_inspect
        _ii_inspector = _ii_inspect(db.engine)
        if 'inbox_email_message' in _ii_inspector.get_table_names():
            _ii_columns = [col['name'] for col in _ii_inspector.get_columns('inbox_email_message')]
            with db.engine.connect() as _ii_conn:
                if 'uid_validity' not in _ii_columns:
                    _ii_conn.execute(text('ALTER TABLE "inbox_email_message" ADD COLUMN uid_validity VARCHAR(64)'))
                if 'is_read' not in _ii_columns:
                    _ii_conn.execute(text('ALTER TABLE "inbox_email_message" ADD COLUMN is_read BOOLEAN DEFAULT FALSE'))
                _ii_conn.commit()
            logging.info("Inbox email message columns verified.")
    except Exception as e:
        logging.warning(f"Could not auto-migrate inbox_email_message columns: {e}")
        try:
            db.session.rollback()
        except:
            pass
...
# Per-account lock: only one sync thread per IMAP account at a time
_inbox_sync_locks = {}  # account_id -> threading.Lock
...
def _get_account_sync_lock(account_id):
    with _inbox_sync_locks_lock:
        if account_id not in _inbox_sync_locks:
            _inbox_sync_locks[account_id] = threading.Lock()
        return _inbox_sync_locks[account_id]
...
def _serialize_imap_account(account):
    ...
    return {
        ...
        'is_enabled': getattr(account, 'is_enabled', True) is not False,
        'auto_sync_enabled': bool(account.auto_sync_enabled),
        'last_synced_at': _gmt_iso(account.last_synced_at),
...
def _enabled_imap_accounts_query():
    return _owned_imap_accounts_query().filter(or_(
        InboxImapAccount.is_enabled.is_(True),
        InboxImapAccount.is_enabled.is_(None),
    ))

def _enabled_imap_account_ids_query():
    return _enabled_imap_accounts_query().with_entities(InboxImapAccount.id)
...
def _imap_list_folders(conn):
    ...
    Returns folder name strings in server order."""
    folders = []
    try:
...
def _imap_select_folder(conn, folder):
    ...
    for item in data or []:
        ...
        try:
            ...
            continue
        m = re.search(r'UIDVALIDITY\s+(\d+)', s, re.I)
        if m and not meta['uidvalidity']:
            meta['uidvalidity'] = m.group(1)
        m = re.search(r'UIDNEXT\s+(\d+)', s, re.I)
        if m and meta['uidnext'] is None:
            meta['uidnext'] = int(m.group(1))
    try:
...
def _upsert_imap_message_full(account, folder, parsed, uid_validity=None):
    ...
    existing.sender_domain = parsed['sender_domain']
    existing.recipient = parsed['recipient']
    existing.subject = parsed['subject']
    existing.preview = parsed['preview']
    existing.plain_text = parsed['plain_text']
    existing.html_content = parsed['html_content']
    existing.headers_json = json.dumps(parsed['headers'])
    existing.x_test_id = parsed['headers'].get('x_test_id') or None
...
def _imap_fetch_messages_batch(conn, uids, folder):
    ...
    fully-parsed message dicts (see _parse_raw_message)."""
    results = []
    if not uids:
        return results
    batch_size = 25
...
def _folder_unread_count(account_id, folder):
    return InboxEmailMessage.query.filter_by(imap_account_id=account_id, folder=folder, is_read=False).count()

def _folder_message_count(account_id, folder):
    return InboxEmailMessage.query.filter_by(imap_account_id=account_id, folder=folder).count()

def _refresh_account_counts(account):
    account.message_count = InboxEmailMessage.query.filter_by(imap_account_id=account.id).count()
    account.inbox_count = InboxEmailMessage.query.filter(
        InboxEmailMessage.imap_account_id == account.id,
        ~InboxEmailMessage.folder.ilike('%spam%'),
        ~InboxEmailMessage.folder.ilike('%junk%'),
        ~InboxEmailMessage.folder.ilike('%bulk%'),
    ).count()
    ...
    account.folder_count = db.session.query(InboxEmailMessage.folder).filter_by(imap_account_id=account.id).distinct().count()
    ...
    try:
        ...
        for folder in target_folders:
            ...
            try:
                ...
                if not state:
                    state = ImapFolderSyncState(imap_account_id=account.id, folder_name=folder)
        ...
        _refresh_account_counts(account)
        db.session.commit()
        app.logger.info(f"[SYNC] {account.email}: +{total_synced} message(s) across {len(folder_results)} folder(s)")
...
def api_inbox_imap_accounts():
    ...
    tls_enabled = _truthy_setting(data.get('tls_enabled'), True)
    auto_sync_enabled = _truthy_setting(data.get('auto_sync_enabled'), True)
    ...
    return _json_no_store({'success': True, 'message': 'Inbox saved.', 'account': _serialize_imap_account(account)})
...
@app.route('/api/inbox-intelligence/imap-accounts/<int:account_id>/settings', methods=['POST'])
@login_required
@permission_required('inbox_intelligence')
def api_inbox_update_account_settings(account_id):
    account = _get_owned_imap_account(account_id)
    if not account:
        return jsonify({'success': False, 'error': 'IMAP account not found'}), 404
    data = request.get_json(silent=True) or {}
...
def api_inbox_sync_account(account_id):
    ...
    if not account:
        return jsonify({'success': False, 'error': 'IMAP account not found'}), 404
    if getattr(account, 'is_enabled', True) is False:
...
def api_explorer_folders():
    ...
    for account in accounts:
        ...
        stored_folders = [
            ...
            .filter_by(imap_account_id=account.id)
            .distinct()
            .all()
            if row[0]
        ]
...
def api_inbox_toggle_auto_sync(account_id):
    ...
    if not account:
        return jsonify({'success': False, 'error': 'IMAP account not found'}), 404
    data = request.get_json(silent=True) or {}
    account.auto_sync_enabled = _truthy_setting(data.get('enabled'), True)
    db.session.commit()
    return jsonify({'success': True, 'account': _serialize_imap_account(account)})
...
def api_inbox_messages():
    ...
    return _json_no_store({'success': True, 'messages': [{
        'id': m.id,
        ...
        'preview': m.preview,
        'uid': m.uid,
        'x_test_id': m.x_test_id,
        'is_read': bool(m.is_read),
        'has_attachments': bool(m.has_attachments),
        'received_at': _gmt_iso(m.received_at),
    } for m in messages]})
...
def api_inbox_message_detail(message_id):
    ...
    return _json_no_store({'success': True, 'message': {
        'id': m.id,
        'imap_account_id': m.imap_account_id,
        'sender': m.sender,
        'recipient': m.recipient,
...
@app.route('/api/inbox-intelligence/messages/<int:message_id>/read', methods=['POST'])
@login_required
@permission_required('inbox_intelligence')
def api_inbox_message_set_read(message_id):
    ...
    m.is_read = read
    db.session.commit()
    return jsonify({'success': True, 'message': 'Marked as read' if read else 'Marked as unread', 'is_read': read})
...
def api_inbox_retrieve_workspace_senders():
    ...
    senders, errors = _retrieve_workspace_senders_from_service_accounts(service_accounts, max_per_account=max_per_account, offset_per_account=offset_per_account)
    return jsonify({'success': True, 'senders': senders, 'errors': errors, 'count': len(senders), 'max_per_account': max_per_account, 'offset_per_account': offset_per_account})
...
def _custom_content_type_main(header_text):
    content_type = _custom_header_value(header_text, 'Content-Type')
    return (content_type.split(';', 1)[0].strip().lower() if content_type else '')

def _custom_content_transfer_encoding(header_text):
    return (_custom_header_value(header_text, 'Content-Transfer-Encoding') or '').strip().lower()

def _custom_content_charset(header_text):
    content_type = _custom_header_value(header_text, 'Content-Type')
...
def _should_send_raw_custom_mime(custom_headers, html_body, text_body):
    ...
    if custom_headers and not _custom_content_type_is_multipart(custom_headers) and (
        _custom_content_transfer_encoding(custom_headers) or _custom_content_type_main(custom_headers)
    ):
        return True
    if not boundary:
        return False
...
def _body_for_declared_content_type(custom_headers, html_body, text_body):
    content_type = _custom_content_type_main(custom_headers)
    if content_type == 'text/plain':
        return text_body if str(text_body or '').strip() else html_utils.unescape(re.sub(r'<[^>]+>', ' ', html_body or ''))
...
def _build_raw_custom_mime_bytes(sender, recipient, subject, html_body, text_body, test_identifier, custom_headers, sender_name=None):
    ...
    if _custom_content_type_is_multipart(custom_headers):
        body = html_body or text_body or ''
    else:
        body = _body_for_declared_content_type(custom_headers, html_body, text_body)
    body = _replace_email_tokens(body, sender, recipient, subject, test_identifier, message_id, sender_name)
    body = _normalize_mime_boundary_delimiters(body, _raw_mime_boundary(custom_headers))
    if _custom_content_type_is_multipart(custom_headers):
        body_bytes = body.encode(_custom_content_charset(custom_headers), errors='replace')
    else:
        body_bytes = _encode_declared_mime_body(body, _custom_content_transfer_encoding(custom_headers), _custom_content_charset(custom_headers))
...
def _saved_analysis_account_for_sender(sender, accounts):
    ...
    return {
        'id': None,
        'name': sender_domain,
        'admin_email': sender_domain,
        'domain': sender_domain,
    }

def _finish_saved_analysis_stat(item, key_name):
    observed = item['inbox'] + item['spam']
    total = observed + item['pending'] + item['failed']
    result = {key_name: item[key_name]}
    result.update({key: item[key] for key in ('inbox', 'spam', 'pending', 'failed')})
    result['observed'] = observed
    result['total'] = total
    result['inbox_rate'] = round((item['inbox'] / observed) * 100, 1) if observed else 0
    result['spam_rate'] = round((item['spam'] / observed) * 100, 1) if observed else 0
    return result

def _build_saved_analysis(test_ids):
    tests = InboxDeliverabilityTest.query.filter(InboxDeliverabilityTest.test_id.in_(test_ids)).all()
    ...
    domain_stats = {}
    account_stats = {}
    totals = {'messages': 0, 'inbox': 0, 'spam': 0, 'pending': 0, 'failed': 0}

    def bump(stats, key, identity, placement):
        item = stats.setdefault(key, {**identity, 'inbox': 0, 'spam': 0, 'pending': 0, 'failed': 0})
        item[placement] += 1
    ...
    for row in rows:
        ...
        if placement == 'INBOX':
            ...
            bucket = 'spam'
        elif placement == 'FAILED' or (row.status or '').strip().upper() == 'FAILED':
            bucket = 'failed'
        else:
            bucket = 'pending'
        totals['messages'] += 1
        totals[bucket] += 1
        bump(user_stats, sender, {
            'email': sender,
            'domain': domain,
            'service_account_id': account['id'],
            'account': account['admin_email'] or account['name'] or domain,
        }, bucket)
        bump(domain_stats, domain, {'domain': domain}, bucket)
        account_key = str(account['id'] or account['admin_email'] or domain)
        bump(account_stats, account_key, {
            'account': account['admin_email'] or account['name'] or domain,
            'account_name': account['name'],
            'service_account_id': account['id'],
            'domain': account['domain'] or domain,
        }, bucket)

    users = []
    for item in user_stats.values():
        result = _finish_saved_analysis_stat(item, 'email')
        result.update({key: item.get(key) for key in ('domain', 'service_account_id', 'account')})
        result['copy_line'] = result['email']
        result['name_change_line'] = f"{result['account']},{result['email']}"
        users.append(result)
    domains = [_finish_saved_analysis_stat(item, 'domain') for item in domain_stats.values()]
    account_rows = []
    for item in account_stats.values():
        result = _finish_saved_analysis_stat(item, 'account')
        result.update({key: item.get(key) for key in ('account_name', 'service_account_id', 'domain')})
    ...
    grey_accounts = sorted((item for item in account_rows if item['inbox'] and item['spam']), key=lambda item: (-item['spam'], item['account']))
    best_domains = sorted((item for item in domains if item['observed']), key=lambda item: (-item['inbox_rate'], -item['observed'], item['domain']))
    observed = totals['inbox'] + totals['spam']
    totals.update({
        'observed': observed,
        'inbox_rate': round((totals['inbox'] / observed) * 100, 1) if observed else 0,
        'spam_rate': round((totals['spam'] / observed) * 100, 1) if observed else 0,
        'users': len(users),
        'domains': len(domains),
    ...
    return {
        ...
        'tests': [{
            ...
            'status': tests_by_id[test_id].status,
            'created_at': tests_by_id[test_id].created_at.isoformat() + 'Z' if tests_by_id[test_id].created_at else None,
...
@app.route('/api/inbox-intelligence/saved-analyses', methods=['GET', 'POST'])
@login_required
@permission_required('inbox_intelligence')
def api_inbox_saved_analyses():
    ...
    if not test_ids:
        return jsonify({'success': False, 'error': 'Select at least one Active or Completed test.'}), 400
    if len(test_ids) > 100:
        return jsonify({'success': False, 'error': 'A saved analysis can include up to 100 tests.'}), 400
    analytics = _build_saved_analysis(test_ids)
    if not analytics['test_ids']:
        return jsonify({'success': False, 'error': 'None of the selected tests exist.'}), 404
    name = str(data.get('name') or '').strip()[:255]
    if not name:
        name = f"Placement analysis - {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}"
    row = InboxSavedAnalysis(
        name=name,
        test_ids_json=json.dumps(analytics['test_ids']),
        analytics_json=json.dumps(analytics),
        created_by=owner,
    )
    db.session.add(row)
    db.session.commit()
    app.logger.info('Saved placement analysis %s from %s test(s)', row.id, len(analytics['test_ids']))
    return jsonify({'success': True, 'analysis': _serialize_saved_analysis(row), 'message': f"Saved analytics for {len(analytics['test_ids'])} test(s)."})

@app.route('/api/inbox-intelligence/saved-analyses/<int:analysis_id>', methods=['DELETE'])
@login_required
@permission_required('inbox_intelligence')
def api_inbox_saved_analysis_delete(analysis_id):
    owner = session.get('user') or ''
...
@app.route('/api/bulk-randomize-user-aliases', methods=['POST'])
@login_required
def api_bulk_randomize_user_aliases():
    """
    ...
    def background_task(task_id, accounts):
        with app.app_context():
            ...
            try:
                app.logger.info(f"[BULK ALIAS TASK {task_id}] Starting for {len(accounts)} accounts")

                # Pre-process account list (supports both string and dict entries)
                account_list = []
                for item in accounts:
                    if isinstance(item, dict):
                        acc_name = item.get('account')
                            ...
                            if not authenticate_without_session(account_name):
                                ...
                                try:
                                    ...
                                    if not sa and not ga:
                                        ...
                                        reason = f"ServiceAccount '{sa.admin_email}' found but auth failed"
                                    elif ga:
                                        reason = f"GoogleAccount '{ga.account_name}' found but auth failed"
                                except Exception:
                                    pass
                                return {'account': account_name, 'authenticated': False, 'error': reason}
                ...
                def randomize_aliases_for_account(account_name):
                    with app.app_context():
                        ...
                        try:
                            ...
                            # Track used alias local names within this run to guarantee uniqueness
                            used_aliases = set()
                            ...
                            for user in all_users:
                                ...
                                # Safeguard: Skip the admin account being used for authentication 
                                # and any other user with admin privileges.
                                if primary_email.lower() == account_name.lower() or user.get('isAdmin'):
                                    app.logger.info(f"[IDENTITY] 🛡️ Skipping admin account: {primary_email}")
                                    continue

                                domain = primary_email.split('@')[1]

                                # Collect existing aliases and primary email to avoid duplicates
                                used_aliases.add(primary_email.split('@')[0].lower())
                                try:
                                    existing = service.users().aliases().list(userKey=primary_email).execute()
                                    for a in existing.get('aliases', []):
                                        alias_email = a.get('alias', '')
                                        if '@' in alias_email:
                                            used_aliases.add(alias_email.split('@')[0].lower())
                                except Exception:
                                    pass
                                ...
                                try:
                                    # Perform a PATCH to change primaryEmail and Name
                                    # This effectively "replaces" the identity.
                                    # Google Workspace will typically keep the old email as an alias.
                                    service.users().patch(
                                    ...
                                    app.logger.info(f"[IDENTITY] ✅ Updated {primary_email} -> {new_primary_email} ({new_first} {new_last})")
                                except Exception as update_err:
                                    account_result['aliases_failed'] += 1
                                    account_result['details'].append({
                                        'user': primary_email,
                                        'attempted_email': new_primary_email,
                                        'success': False,
                                        'error': str(update_err)
                                    })
                                    app.logger.warning(f"[IDENTITY] ❌ Failed update for {primary_email}: {update_err}")

                        except Exception as e:
                            account_result['error'] = str(e)
                            app.logger.error(f"[ALIAS] ✗ Account {account_name} crashed: {e}")

                        return account_result

                # Run alias randomization in parallel across accounts
                with ThreadPoolExecutor(max_workers=max(1, min(10, len(authenticated_accounts)))) as alias_executor:
                    alias_futures = {alias_executor.submit(randomize_aliases_for_account, acc): acc for acc in authenticated_accounts.keys()}
                    completed_count = 0
                    for future in as_completed(alias_futures):
                        res = future.result()
                        all_results.append(res)
                        completed_count += 1
                        update_progress(task_id, completed_count, len(authenticated_accounts), "randomizing",
                                        f"Processed {completed_count}/{len(authenticated_accounts)} accounts...")

                # Add auth failures to results
                for acc in account_list:
                    if acc not in authenticated_accounts:
                        all_results.append({
                            'account': acc,
                ...
                total_aliases_added = sum(r.get('aliases_added', 0) for r in all_results)
                total_aliases_failed = sum(r.get('aliases_failed', 0) for r in all_results)
                total_users = sum(r.get('user_count', 0) for r in all_results)
                ...
                final_result = {
                    ...
                    'total_aliases_failed': total_aliases_failed,
                    'results': all_results
                }

                update_progress(task_id, 100, 100, "completed", "Bulk alias randomization finished!", final_result)
                app.logger.info(f"[BULK ALIAS TASK {task_id}] COMPLETED — {total_aliases_added} aliases added")

            except Exception as e:
                app.logger.error(f"[BULK ALIAS TASK {task_id}] CRASHED: {e}")
                app.logger.error(traceback.format_exc())
                update_progress(task_id, 0, 0, "error", f"Task failed: {str(e)}")
...
def _normalize_targeted_alias_pairs(raw_targets):
    normalized = []
    errors = []
    seen = set()
    ...
    for index, item in enumerate(raw_targets or [], start=1):
        ...
        if isinstance(item, dict):
            account = str(item.get('account') or '').strip()
            user_key = str(item.get('user') or item.get('user_key') or '').strip()
        elif isinstance(item, str):
            ...
            if len(parts) == 2:
                ...
                errors.append(f"Line {index}: expected account,user")
                continue
        else:
            errors.append(f"Line {index}: invalid target format")
            continue

        if not account or '@' not in account:
            errors.append(f"Line {index}: admin account must be a full email address")
            continue
        if not user_key:
            errors.append(f"Line {index}: target user is required")
            continue

        if '@' not in user_key:
            user_key = f"{user_key}@{account.split('@', 1)[1]}"

        dedupe_key = f"{account.lower()}|{user_key.lower()}"
        if dedupe_key in seen:
            continue
        seen.add(dedupe_key)
        normalized.append({'account': account, 'user': user_key})
...
@app.route('/api/targeted-randomize-user-aliases', methods=['POST'])
@login_required
def api_targeted_randomize_user_aliases():
    """
    Authenticate to the requested admin accounts and randomize only the specific
    Workspace users provided as account,user pairs.
    """
    cleanup_old_progress()
    data = request.get_json(silent=True) or {}
    ...
    def background_task(task_id, account_targets, resolution_failures):
        with app.app_context():
            ...
            try:
                ...
                update_progress(task_id, 0, len(account_names), "authenticating", f"Authenticating {len(account_names)} account(s)...")
                authenticated_accounts = {}
                auth_failures = []

                with ThreadPoolExecutor(max_workers=max(1, min(10, len(account_names)))) as auth_executor:
                    auth_futures = {auth_executor.submit(_authenticate_workspace_alias_account, acc): acc for acc in account_names}
                    for i, future in enumerate(as_completed(auth_futures)):
                        account_name = auth_futures[future]
                        res = future.result()
                        if res.get('authenticated'):
                            authenticated_accounts[account_name] = res
                        else:
                            auth_failures.append(res)
                        update_progress(task_id, i + 1, len(account_names), "authenticating", f"Authenticated {i + 1}/{len(account_names)} account(s)...")

                for failure in auth_failures:
                    failed_users = account_targets.get(failure.get('account'), [])
                    all_results.append({
                        'account': failure.get('account'),
                        'authenticated': False,
                        'error': failure.get('error') or 'Authentication failed',
                        'aliases_added': 0,
                        'aliases_failed': len(failed_users),
                        'skipped': 0,
                        'user_count': len(failed_users),
                        'details': [
                            {
                                'user': user_key,
                                'success': False,
                            ...
                            for user_key in failed_users
                        ]
                    })

                if not authenticated_accounts:
                    first_err = auth_failures[0].get('error') if auth_failures else "Unknown"
                    result_data = {
                        'success': False,
                        'error': f"All {len(account_names)} accounts failed auth. First error: {first_err}",
                        'results': all_results
                    }
                ...
                with ThreadPoolExecutor(max_workers=max(1, min(10, len(authenticated_accounts)))) as alias_executor:
                    alias_futures = {
                        alias_executor.submit(process_account_targets, acc, account_targets[acc]): acc
                        for acc in authenticated_accounts.keys()
                    }
                    for future in as_completed(alias_futures):
                        res = future.result()
                        all_results.append(res)
                        processed_users += res.get('user_count', 0)
                        update_progress(task_id, min(processed_users, len(targets)), len(targets), "randomizing",
                                        f"Processed {min(processed_users, len(targets))}/{len(targets)} selected user(s)...")
...
@app.route('/api/domain-gen-config', methods=['GET'])
@login_required
def get_domain_gen_config():
    try:
        from database import DomainGenConfig
        config = DomainGenConfig.query.first()
        if config:
            return jsonify({'success': True, 'tlds': config.tlds.split(',')})
        else:
            # Return default list from service
            from services.domain_generator_service import DomainGeneratorService
            return jsonify({'success': True, 'tlds': DomainGeneratorService.TLDS})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/domain-gen-config', methods=['POST'])
@login_required
def save_domain_gen_config():
    try:
        data = request.get_json()
        tlds = data.get('tlds', [])
        if not tlds:
            return jsonify({'success': False, 'error': 'TLD list cannot be empty'})
        
        # Clean TLDs (ensure they start with dot if needed, though service handles it)
        cleaned_tlds = []
        for t in tlds:
            t = t.strip().lower()
            if not t.startswith('.'):
                t = '.' + t
            cleaned_tlds.append(t)
            
        from database import DomainGenConfig
        config = DomainGenConfig.query.first()
        if not config:
            config = DomainGenConfig(tlds=','.join(cleaned_tlds))
            db.session.add(config)
        else:
            config.tlds = ','.join(cleaned_tlds)
            
        db.session.commit()
        return jsonify({'success': True, 'message': 'TLD configuration saved'})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})

@app.route('/api/generate-niche-domains', methods=['POST'])
@login_required
def generate_niche_domains():
    """Generate niche-specific domains, check availability, and notify via Telegram."""
    try:
        from services.domain_generator_service import DomainGeneratorService
        ...
        TELEGRAM_TOKEN = "8696839407:AAFk77h2FY0yMZ0QBXuIKjrlqoJNT1nvLy0"
        
        gen_service = DomainGeneratorService(TELEGRAM_TOKEN)
        
        # Initialize Namecheap service
        try:
            nc_service = NamecheapDNSService()
        except Exception as nc_err:
            return jsonify({'success': False, 'error': f"Namecheap not configured: {str(nc_err)}"})
            
        # Get count and custom TLDs from request
        data = request.get_json() or {}
        target_count = data.get('count', 40)
        
        # Ensure count is within reasonable limits
        try:
            target_count = int(target_count)
            if target_count <= 0: target_count = 40
            if target_count > 100: target_count = 100 
        except:
            target_count = 40

        # Load custom TLDs if provided, otherwise check database, otherwise use defaults
        custom_tlds = data.get('tlds')
        if not custom_tlds:
            db_config = DomainGenConfig.query.first()
            if db_config:
                custom_tlds = db_config.tlds.split(',')
        ...
        while len(available_domains) < 5 and attempt < max_attempts:
            attempt += 1
            app.logger.info(f"[DOMAIN GEN] Attempt {attempt}: Generating {target_count} domains...")
            
            # Generate fresh batch
            batch = gen_service.generate_domains(
                niches=['education', 'industry', 'ai', 'tech'], 
                count=target_count,
                tlds=custom_tlds
            )
            
            # Filter out domains we already checked in this session
            new_domains = [d for d in batch if d not in already_checked]
            if not new_domains:
                continue
                
            for d in new_domains: already_checked.add(d)

            # 2. Check Namecheap for availability
            app.logger.info(f"[DOMAIN GEN] Checking availability for {len(new_domains)} domains...")
            availability_results = nc_service.check_availability(new_domains)
            
            new_available = [r['domain'] for r in availability_results if r['available'] and not r['premium']]
            available_domains.extend(new_available)
            
            app.logger.info(f"[DOMAIN GEN] Found {len(new_available)} available domains in this batch. Total: {len(available_domains)}")
            
            if len(available_domains) >= 5:
                break
        
        if not available_domains:
            app.logger.info("[DOMAIN GEN] No available domains found after multiple attempts.")
            return jsonify({'success': True, 'message': 'No available domains found after several attempts.', 'domains': []})
            
        # 3. Find Chat ID from bot updates
        app.logger.info("[DOMAIN GEN] Fetching Telegram bot updates for Chat ID...")
        updates = gen_service.get_bot_updates()
        chat_id = None
        ...
        if not chat_id:
             app.logger.warning("[DOMAIN GEN] Could not find a Chat ID via getUpdates")
             return jsonify({
                 'success': False, 
                 'error': 'Could not find a Chat ID. Please send a message to your Telegram bot first so it can identify you.'
             })

        # 4. Send to Telegram
        message = "<b>🚀 Available Longtail Domains Found:</b>\n\n"
        sorted_domains = sorted(list(set(available_domains))) # Unique and sorted
        for d in sorted_domains:
            message += f"• <code>{d}</code>\n"
        
        message += f"\nTotal: <b>{len(sorted_domains)}</b> available domains found."
        message += f"\n<i>Attempted batches: {attempt}</i>"
        message += "\n<i>Niches: Education, Industry, AI, Tech</i>"
        
        app.logger.info(f"[DOMAIN GEN] Sending {len(sorted_domains)} domains to Chat ID {chat_id}...")
        gen_service.send_to_telegram(chat_id, message)
        
        return jsonify({
            'success': True, 
            'message': f'Successfully found {len(available_domains)} available domains and sent them to your Telegram bot.',
            'domains': available_domains
        })

    except Exception as e:
        app.logger.error(f"CRITICAL ERROR in generate_niche_domains: {e}")
        app.logger.error(traceback.format_exc())
        return jsonify({'success': False, 'error': str(e)})
...
def api_test_proxies():
    ...
    # Broaden role check or log it
    current_role = str(session.get('role', '')).lower()
    if current_role not in ['admin', 'support']:
        app.logger.warning(f"Unauthorized proxy test attempt by role: {current_role}")
        return jsonify({'success': False, 'error': f'Unauthorized. Current role: {current_role}'})
    ...
    try:
        ...
        app.logger.info(f"Starting proxy test for {len(proxy_lines)} proxies...")
        
        for line in proxy_lines:
            try:
                # Handle multiple formats: IP:PORT or IP:PORT:USER:PASS
                parts = line.split(':')
                proxies = None
                ...
                if len(parts) == 4:
                    ...
                    results.append({'proxy': line, 'status': 'Error', 'message': 'Invalid format. Use IP:PORT or IP:PORT:USER:PASS'})
                    continue
                
                # Try test URL
                success = False
                test_urls = ["http://ip-api.com/json", "https://api64.ipify.org?format=json"]
                ...
                for test_url in test_urls:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            detected_ip = ip_data.get('query') or ip_data.get('ip') or 'Unknown'
                            country = ip_data.get('country') or 'N/A'
                            city = ip_data.get('city') or 'N/A'
                            ...
                            success = True
                            app.logger.info(f"Proxy {line} SUCCESS: {detected_ip}")
                            break
                    except Exception as inner_e:
                        app.logger.debug(f"Proxy {line} failed test URL {test_url}: {str(inner_e)}")
                        continue
                
                if not success:
                    results.append({'proxy': line, 'status': 'Failed', 'message': 'Connection timeout or proxy refused connection'})
                    app.logger.warning(f"Proxy {line} FAILED all test URLs")
                    
            except Exception as e:
                results.append({'proxy': line, 'status': 'Failed', 'message': str(e)})
                app.logger.error(f"Error testing proxy {line}: {str(e)}")
        ...
        app.logger.error(f"Global error in api_test_proxies: {str(e)}")
        return jsonify({'success': False, 'error': str(e)})
