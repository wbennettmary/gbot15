...
class NamecheapDNSService:
    ...
    def check_availability(self, domains: List[str]) -> List[Dict]:
        ...
        Returns:
            List of dictionaries with domain name and availability status
        """
        if not domains:
            return []
            
        try:
            logger.info(f"Checking availability for {len(domains)} domains via Namecheap...")
            
            # Namecheap allows checking up to 50 domains at once
            results = []
            for i in range(0, len(domains), 50):
                batch = domains[i:i+50]
                result = self._make_request('namecheap.domains.check', {
                    'DomainList': ','.join(batch)
                })
                
                root = result['xml']
                # Path: CommandResponse/DomainCheckResult
                for check_result in root.findall('.//DomainCheckResult'):
                    domain_name = check_result.get('Domain', '')
                    is_available = check_result.get('Available', 'false').lower() == 'true'
                    is_premium = check_result.get('IsPremiumName', 'false').lower() == 'true'
            ...
            logger.info(f"Availability check complete. Found {len([r for r in results if r['available']])} available domains.")
            return results
