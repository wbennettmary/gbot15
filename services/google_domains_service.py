...
class GoogleDomainsService:
                    ...
                    try:
                        ...
                        if status == 503:
                            if attempt < max_retries - 1:
                                ...
                                # A 503 backendError means Google's backend may have
                                # processed the insert without returning a response.
                                # The domain is already added and the DNS TXT record
                                # is set, so this is verification pending, NOT failed.
                                logger.warning(f"⚠️ 503 backendError after {max_retries} retries for {verification_domain}; insert may have been processed server-side")
                                if self._site_verification_present(verification_domain):
                                    logger.info(f"✅ {verification_domain} present in Site Verification resources despite the 503s")
                                    self._ensure_workspace_admin_owner(verification_domain)
                                return {
                                    'verified': True,
                                    'status': 'syncing',
                                    'error': f'Site Verification request submitted for {verification_domain}; Google returned a transient 503 backend error but the domain is added and the DNS TXT record is set. Verification completes once Google\'s backend recovers.'
                                }
        ...
        try:
            ...
            for check in range(max_sync_checks):
                try:
                    ...
                    if e.resp.status == 404:
                        ...
                        logger.info(f"Domain {verification_domain} not found in Workspace (404) during check {check+1}, will retry")
                        if check < max_sync_checks - 1:
                            time.sleep(5)
                            continue
            ...
            if domain_not_found:
                ...
                try:
                    ...
                    # Site Verification already succeeded, so treat this as verified
                    # and let the UI know the Workspace record is syncing.
                    return {
                        'verified': True,
                        'status': 'syncing',
                        'error': 'Domain verified via Site Verification. Workspace status syncing - may take a few minutes to reflect.'
                    }
            ...
            # If we exhausted the loop and it's still not verified, Site Verification
            # already succeeded so the domain IS verified; Workspace just hasn't
            # synced yet. Report success rather than a false failure.
            if not current_verified:
                ...
                return {
                    'verified': True, 
                    'status': 'syncing', 
                    'error': 'Site Verification succeeded; Google Workspace Admin is still syncing verification status.'
                }
    ...
    def _site_verification_present(self, domain: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"Could not list Site Verification resources for {domain}: {e}")
        return False
